...
戴相机的人在屋子里怎么走动、两只手到过哪些位置、在那儿做了什么 —— 而且要的是它们在真实空间里的位置，不是画面里的像素位置。想让机器看懂一个人在做什么、想让机器人照着人学动作、想把虚拟物体稳稳钉在真实房间里，第一步都得先拿到这些。但今天要拿到它，代价很高：要么买带 SLAM 的专业采集设备，要么把相机标定 → 单目深度 → SLAM → 手部重建 → 轨迹清理串成一条链，每一级下载一份权重、每一级排查一类失败、每一级承担一条许可证限制。结果是产出这类数据的能力，集中在少数跑得起这条链的实验室手里 —— 而具身智能最缺的，恰恰是海量的真实人手数据。

MINT 就是为补上这个缺口而做的：**一段普通 RGB 视频、一张 24 GB 显存的显卡、一次前向**，就能得到世界坐标系下的相机与双手运动。不出深度图、不出点云、不做逐序列优化，也不需要在自己机器上把那条五级链重新搭起来。有视频，就能产数据。

- **一个统一模型，而不是五级串行链。** 共享的时空表征同时驱动相机外参头、独立视场角头、相机坐标系 MANO 头和逐帧手部存在性头，再由显式可微的刚体组合得到世界坐标系手部运动。推理时不需要任何稠密 3D 中间结果。
- **结构化管线摊销（structured pipeline amortization）。** 多级管线保留下来 —— 但放到线下，只作为监督信号的生成器 —— 然后用一个模型去学它最终那份结构化状态。这不是 logit 蒸馏：学生学的是一整套非端到端系统输出的结构化相机–手部状态。跑这套系统的代价由我们付一次，而不是每个使用者各付一次。
- **端到端开源。** 模型权重、训练与推理代码、EgoPipeline 标注系统，以及经过严格过滤的 **1,021 小时**结构化第一视角数据集 —— 整条链可复现、可审查、可以自己接着改，而不是一个只能调用的服务。
...
推理吞吐，512 × 384、30 fps，条件完全一致。时间为**稳态下的每帧边际成本**。加速比一列以 **VITRA** 为基准，原因是本项目的数据管线 **EgoPipeline** 正是在 VITRA 的基础上优化而来的 —— VITRA 是这条路线的起点。所以这些行是同一条演进线，而不是一堆竞争者：VITRA 是出发点，EgoPipeline 是对它做的管线级优化，MINT 则用一个统一模型替掉整条链。
...
以 VITRA 为基准，MINT 单卡快 **17.4 倍**（1260.0 → 72.4 ms/帧），四卡快 **12.5 倍**（283.3 → 22.7 ms/帧）。四卡这一组还能看出增益的来处：管线级优化把 VITRA 的 283.3 ms/帧降到 EgoPipeline 的 83.4（3.4×），再用一个统一模型替掉整条链，降到 22.7（合计 12.5×）。四卡下 MINT 也是全表唯一跑过 30 fps 实时线的一行（44.1 fps）。
...
论文 Table 1 原样搬运。评分方式会把「不输出」也算成错误：某只手如果方法拒绝给结果，不会被跳过，而是按一个标准 MANO 占位手的误差计入 —— 所以不预测永远不会比预测得差更划算。**MINT 在两个 benchmark 上都是零样本** —— 训练时从没见过它们。ViDiHand（标 `*`）在这两个 benchmark 的大部分数据上训练过，所以它**只列作参考**，评判谁最好时不算它；加粗标的是其余各行中的最优值。`MINT + UKF` 是同一个模型，只是推理时打开了滤波，别的都没变。
...
这张表要和[已知局限](#️-已知局限)里关于手部精度那一条一起读。在 **HOT3D** 上，除了只作参考的 ViDiHand，MINT 在检测三项以及 MPJPE-p、PA-MPJPE-p、GO-p、CT-p 上都是表里最好的，`MINT + UKF` 的 jitter 更是全表最低。在 **ARCTIC** 上，它在相机系下的关节误差明显落后于那些只做手部重建的专用方法 —— 原因是这版 checkpoint 的手部分支只在 EgoPipeline 产出的粗糙标签上训练过，从未用高精度手部数据微调，所以它的手部精度跟随的是这批标签的质量。两个 benchmark 我们都报，而不是只挑好看的那个。
...
**对这个模型来说，真正重要的指标是 RPE-T 和 RPE-R。** MINT 存在的目的是产出第一视角标注，这些标注被当作训练数据消费，而训练吃的是逐帧的相机运动增量（delta），不是全局坐标系下的绝对位姿。所以相对位姿误差才是会传导到下游训练里的量；ATE 度量的是累积的全局漂移，而一个只吃 delta 的训练流程根本看不到它。

在这两项上：**ARCTIC** 上 MINT 的 RPE-T 是全表最低 —— 均值 3.39 mm，而最好的外部方法是 8.73 mm；RPE-R 均值在所有外部方法中也是最低（0.256 对 HaWoR 的 0.298）。**HOT3D** 上 RPE-T 以 4.69 mm 排第二，落后于 MegaSaM 的 3.18 mm，而 MegaSaM 只跑完了 27 条中的 24 条；该数据集上的 RPE-R 则落后于 MegaSaM 与 DROID-SLAM。

另一面也直说：**ATE 不是本版 checkpoint 占优的地方** —— HOT3D 上 181.7 mm，而 DROID-SLAM 是 49.1 mm —— 弧长比说明了原因。HOT3D 上 1.094、ARCTIC 上 1.412，意味着预测出的路径比真实路径更长，这正是[已知局限](#️-已知局限)里记下的尺度放大。Stage 2 就是用来处理这件事的：没有 Stage 2 时，HOT3D 上的弧长比是 0.466，短得很厉害，Stage 2 把它拉到 1.094 —— 而在 ARCTIC 上则校正过头了。绝对米制尺度是本次发布里尚未解决的问题；相对运动不是。
...
- **训练是在 32 帧片段上做的，这本身不能证明长视频上的一致性。** 长序列的表现我们单独报告，不拿片段级的精度去外推。
